...
в tKZwbsP.class идёт подгрузка модов из папки C:\Xenoceal\mods, также разрешена модификация пакетов для PluginMessage (убрано использование метода slice в ByteBuf)
в FafXKPU.class убираются проверки на доступ к внутренним классам майнкрафта   
pUwrrMT.class это интерфейс класса FafXKPU.class, иначе без него не запускается майнкрафт
