...
в axG.class идёт подгрузка модов из папки C:\Xenoceal\mods, также разрешена модификация пакетов для PluginMessage (убрано использование метода slice в ByteBuf)
в si.class убираются проверки на доступ к внутренним классам майнкрафта   
ke.class это интерфейс класса si.class, иначе без него не запускается майнкрафт
